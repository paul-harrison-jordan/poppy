...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [
        { role: 'system', content: AgentRegistry.getPrompt('engineering') },
        { 
          role: 'user', 
    ...
    console.error('Engineering agent error:', error);
    return NextResponse.json(
      { error: 'Engineering agent failed', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}