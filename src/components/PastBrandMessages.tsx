...
export default function PastBrandMessages() {
  const [isExpanded, setIsExpanded] = useState(false);
  const [brandMessages, setBrandMessages] = useState<BrandMessage[]>([]);
  const [deletingId, setDeletingId] = useState<string | null>(null);

  useEffect(() => {
    const stored = localStorage.getItem('brandMessages');
    if (stored) {
      try {
        const parsedMessages = JSON.parse(stored);
        setBrandMessages(parsedMessages);
      } catch (error) {
        console.error('Error parsing brand messages:', error);
      }
    }
  }, []);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      // Remove from state
      const updatedMessages = brandMessages.filter(message => message.id !== id);
      setBrandMessages(updatedMessages);
      
      // Update localStorage
      localStorage.setItem('brandMessages', JSON.stringify(updatedMessages));
    } catch (error) {
  ...
  return (
    <div className="mt-8">
      ...
      >
        <span className="text-lg font-semibold text-[#232426]">Past Work</span>
        <svg
              ...
              {brandMessages.map((message) => (
                <tr 
                  ...
                  className={`transition-all duration-200 ${
                    deletingId === message.id ? 'opacity-50' : ''
                  }`}
                >
